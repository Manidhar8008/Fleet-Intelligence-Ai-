...
def _calculate_zone_pressure(df: pd.DataFrame) -> pd.Series:
    ...
    max_count = zone_counts.max() if len(zone_counts) > 0 else 1
    ...
    df.drop('zone_vehicle_count', axis=1, inplace=True)
...
def get_feature_statistics(df: pd.DataFrame) -> dict:
    ...
    """
    stats = {
        'avg_battery': float(df['battery'].mean()) if len(df) > 0 else 50,
        'avg_utilization': float(df['utilization'].mean()) if len(df) > 0 else 50,
        'maintenance_overdue_count': int(df['maintenance_due'].sum()) if 'maintenance_due' in df.columns and len(df) > 0 else 0,
